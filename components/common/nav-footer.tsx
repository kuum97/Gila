...
export default function FooterNavigation({ routes }: Props) {
  return (
    <footer className="tall:sticky fixed inset-x-0 bottom-0 z-50 flex items-center justify-around w-full overflow-x-auto bg-white border-t bg-opacity-95 tall:max-w-[420px]">
      <div className="flex">
        {routes.map((route) => (
          <NavIconLink
            key={route.label}
